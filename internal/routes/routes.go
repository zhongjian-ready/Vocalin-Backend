...
func SetupRouter(application *app.App) (*gin.Engine, error) {
	...
	api.Use(middleware.AuthMiddleware(application.TokenManager))
	{
		...
		api.PUT("/records/notes/:id/folder", recordHandler.MoveNoteToFolder)
		api.PUT("/records/notes/:id/visibility", recordHandler.UpdateNoteVisibility)
		api.DELETE("/records/notes/:id", recordHandler.DeleteNote)
		api.GET("/records/notes", recordHandler.GetNotes)
		api.POST("/records/note-folders", recordHandler.CreateNoteFolder)
		api.PUT("/records/note-folders/:id", recordHandler.UpdateNoteFolder)
		api.DELETE("/records/note-folders/:id", recordHandler.DeleteNoteFolder)
		api.GET("/records/note-folders", recordHandler.GetNoteFolders)
		api.POST("/records/wishlist", recordHandler.CreateWishlist)
		api.PUT("/records/wishlist/:id", recordHandler.UpdateWishlist)
		api.DELETE("/records/wishlist/:id", recordHandler.DeleteWishlist)
